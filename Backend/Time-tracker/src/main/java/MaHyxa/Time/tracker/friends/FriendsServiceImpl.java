...
public class FriendsServiceImpl implements FriendsService {
    ...
    @Override
    public ResponseEntity<String> requestConnect(JsonNode email, Authentication connectedUser) {
        ...
        if (requestedBy != null && user != null) {
            ...
            if (isFriends(user, requestedBy) == RelationshipStatus.PENDING) {
                ...
                log.info("User=[{}] requested connection to user=[{}]", requestedBy, user);
    ...
    public ResponseEntity<?> acceptConnect(JsonNode requestBody, Authentication connectedUser) {
        ...
        try {
            if (found.isPresent()) {
                ...
                log.info("User=[{}] accepted connection from user=[{}]", user, requestedBy);
    ...
    public ResponseEntity<?> rejectConnect(JsonNode requestBody, Authentication connectedUser) {
        ...
        try {
            if (found.isPresent()) {
                ...
                log.info("Connection between User1=[{}] and User2=[{}] removed.", user, requestedBy);
                ...
                if (found.get().getRequestedBy().equals(user)) {
                    notificationService.sendNotification(user,
                            Notification.builder()
                                    ...
                                    .message("Connection with " + keycloakService.getUserEmailById(requestedBy) + " removed.")
                                    .build());
                } else {
                    notificationService.sendNotification(requestedBy,
                            Notification.builder()
